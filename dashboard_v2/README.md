...
| `NEXT_PUBLIC_DASHBOARD_WALLET_ADDRESS` | client | `0x…` wallet address for the Overview **revenueSummary** query (required field on the API). Valid hex addresses are normalized to **EIP-55 checksum** before calling GraphQL (many backends key the ledger by checksummed address). |
